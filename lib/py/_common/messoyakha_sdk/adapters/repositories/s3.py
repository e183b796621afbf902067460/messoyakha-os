...
class S3PolarsRepositoryBase(ABC):
    ...
    def __attrs_post_init__(self) -> None:
        self._s3_filesystem: S3FileSystem = S3FileSystem(
        ...
        self._arrow_filesystem: PyFileSystem = PyFileSystem(FSSpecHandler(self._s3_filesystem))

    def _query(self, query: str) -> LazyFrame:
        return self._context.execute(query=query, eager=False)

    def _truncate(self, path: str) -> None:
        if self._s3_filesystem.exists(path):
            self._s3_filesystem.rm(path, recursive=True)
