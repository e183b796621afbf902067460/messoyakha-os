...
Hermes Agent runs inside a docker container: paths from the host machine (e.g. `/home/user/.../messoyakha-os/...`) may appear in tasks, attachments, or configs. When resolving them, match only the part starting from `messoyakha-os/` and look it up under the container mount point `/code/...` (the project root), i.e. `/code/...`.
